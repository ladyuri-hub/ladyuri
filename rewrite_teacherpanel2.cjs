...
// 3. Remove stray variable declarations that might have been left over
code = code.replace(/let restoredCount = 0;\s*for \(let i = 1; i < lines\.length; i\+\+\) \{[\s\S]*?alert\('복구할 데이터 형식을 찾을 수 없습니다\.'\);\s*\}/g, '');


// 4. Re-insert them once, properly
const cleanCode = `
  const handleRestoreBriefingCSV = (e: any) => {
    ...
    reader.onload = (evt) => {
      ...
      if (rCount > 0) {
        if (confirm(\`총 \${rCount}건의 설명회 참석 데이터를 복구하시겠습니까?\`)) {
  ...
  const handleRestoreCSV = (e: any) => {
    ...
    reader.onload = (evt) => {
      ...
      if (rCount > 0) {
        if (confirm(\`총 \${rCount}건의 예약 데이터를 복구하시겠습니까?\`)) {
