...
const LookupModal = ({ onClose }: any) => {
  ...
  const [lookupMode, setLookupMode] = useState<'simple' | 'find_pw'>('simple');
      ...
      for (const d of settings.dates) {
        for (const t of settings.times) {
          ...
          if (b && b.studentName === name && b.phone && b.phone.endsWith(phone)) {
            found.push({ date: d.label, time: t, day: d.day, ...b, _mode: 'simple' });
          }
        }
      }
      ...
      for (const d of settings.dates) {
        for (const t of settings.times) {
          ...
          if (b && b.studentName === name && b.parentName === parentName && b.phone === phone) {
            found.push({ date: d.label, time: t, day: d.day, ...b, _mode: 'find_pw' });
          }
        }
      }
    }
        ...
        <div className="p-5 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          {lookupMode === 'simple' && (
            <div>
              ...
              <p className="text-[10px] text-emerald-600 mt-1 font-medium">* 예약 접수 여부와 시간만 확인할 수 있습니다.</p>
            </div>
          )}
          ...
          {searched && (
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs space-y-2">
              {result.length > 0 ? result.map((r, i) => (
                <div key={i} className="border-b border-slate-200 pb-3 mb-3 last:border-0 last:pb-0 last:mb-0">
                  <p className="font-bold text-blue-700 mb-1">{r.date}({r.day}) {r.time}</p>
                  ...
                  {r._mode === 'simple' && (
                     <div className="mt-2 p-2 bg-emerald-100/50 rounded border border-emerald-200 text-[10px] text-emerald-800 font-medium">
                       <strong>[안내]</strong> 예약 접수 완료<br/>예약 관련 변경 사항은 담임 교사에게 문의해 주세요.
                     </div>
                  )}

                  {r._mode === 'find_pw' && (
                     <div className="mt-2 p-3 bg-rose-50 rounded-lg border border-rose-200 text-xs text-rose-800 font-medium">
                       해당 예약의 비밀번호는 <strong className="text-rose-600 text-sm">{r.password}</strong> 입니다.
                     </div>
                  )}
                </div>
              )) : (
              ...
              <div className="mt-3 text-center">
                <p className="text-[10px] text-slate-500 font-medium">예약 취소 및 변경은 담임 교사에게 문의해 주세요.</p>
              </div>
            </div>
          )}
        </div>
